...
export const getStateOrderDocuments = state => state.api.wyatt['order-document'];
...
export const getResourcesByFilter = createSelector(
  [getPredicate, getStateResources],
  (filters, resources) => _.uniqBy(_.reduce(filters, (result, filter) => _.concat(result, _.filter(resources, filter)), []), 'uri'),
);
...
export const getPrintsWithNames = createSelector(
  ...
  (prints, resources) => prints.map((print) => {
...
export const getLastTenOrders = createSelector(
  [getOrders],
  orders => _.takeRight(_.sortBy(orders, order => moment(order.created).local()), 10),
);
